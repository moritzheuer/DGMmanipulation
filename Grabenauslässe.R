...
df <- read_csv("S:/DGM_Flow_Modellierung/07_data_preprocessing/QGIS_Projekte/Grabenverschlüsse_Wege_Vertices_Koordinaten.csv")
Auflösung <- 0.5
df <- df[df$Type == "Weg",]
df$id <- df$id - 65
df <- df[order(df$id),]
df <- df[,c("id", "vertex_index", "xcoord", "ycoord")]
...
colnames(df) <- c("id", "vertex_index", "column", "row")
...
for (id in sort(unique(df$id))) {
  partDrainage <- bresenham(x = c(df[df$id == id & df$vertex_index == 2,]$column,
                                  df[df$id == id & df$vertex_index == 1,]$column),
                            y = c(df[df$id == id & df$vertex_index == 2,]$row,
                                  df[df$id == id & df$vertex_index == 1,]$row))
...
writeRaster(dem,
            "S:/DGM_Flow_Modellierung/07_data_preprocessing/QGIS_Projekte/Argenschwang_Wegegräben_Verschluss.tif",
...
df <- read_csv("S:/DGM_Flow_Modellierung/07_data_preprocessing/QGIS_Projekte/Wegeausleitungen_Vertices_Koordinaten.csv")
...
for (id in sort(unique(df$id))) {
  partDrainage <- bresenham(x = c(df[df$id == id & df$Type == "Akkumulator" & df$vertex_index == 2,]$column,
                                  df[df$id == id & df$Type == "Akkumulator" & df$vertex_index == 1,]$column),
                            y = c(df[df$id == id & df$Type == "Akkumulator" & df$vertex_index == 2,]$row,
                                  df[df$id == id & df$Type == "Akkumulator" & df$vertex_index == 1,]$row))
  for (index in (2:(max(df[df$id == id & df$Type == "Akkumulator",]$vertex_index)-1))) {
...
for (id in sort(unique(df$id))) {
  partDrainage <- bresenham(x = c(df[df$id == id & df$Type == "Distributor" & df$vertex_index == 2,]$column,
                                  df[df$id == id & df$Type == "Distributor" & df$vertex_index == 1,]$column),
                            y = c(df[df$id == id & df$Type == "Distributor" & df$vertex_index == 2,]$row,
                                  df[df$id == id & df$Type == "Distributor" & df$vertex_index == 1,]$row))
  for (index in (2:(max(df[df$id == id & df$Type == "Distributor",]$vertex_index)-1))) {
...
for (id in sort(unique(df$id))) {
  partDrainage <- bresenham(x = c(df[df$id == id & df$Type == "Endgraben" & df$vertex_index == 2,]$column,
                                  df[df$id == id & df$Type == "Endgraben" & df$vertex_index == 1,]$column),
                            y = c(df[df$id == id & df$Type == "Endgraben" & df$vertex_index == 2,]$row,
                                  df[df$id == id & df$Type == "Endgraben" & df$vertex_index == 1,]$row))
  for (index in (2:(max(df[df$id == id & df$Type == "Endgraben",]$vertex_index)-1))) {
...
for (k in 1:length(grabenList)) {
  ...
  for (i in 1:nrow(zellen_Wall)) {
    dem[zellen_Wall$y[i], zellen_Wall$x[i]] <- (dem[zellen_Wall$y[i], zellen_Wall$x[i]] + 0.35)
  }
}
...
writeRaster(dem,
            "S:/DGM_Flow_Modellierung/07_data_preprocessing/QGIS_Projekte/Argenschwang_Wege_Ausleitungen.tif",
            options = c("COMPRESS=NONE"),
            overwrite = TRUE,
            datatype = "FLT8S")
