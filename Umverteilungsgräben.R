...
id_list <- unique(dx$id)
for (graben in 1:length(verschlussList)) {
  for (zelle in 1:nrow(verschlussList[[graben]])) {
    dem[verschlussList[[graben]][zelle, "y"], verschlussList[[graben]][zelle, "x"]] <- max(dx[dx$id == id_list[graben], "height"])
  }
}
#for (graben in 1:length(verschlussList)) {
#  for (zelle in 1:nrow(verschlussList[[graben]])) {
#    print(dx[dx$id == graben, "height"])
