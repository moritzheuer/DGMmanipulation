...
df <- read_csv("S:/DGM_Flow_Modellierung/07_data_preprocessing/Tabellen/Keylines_angepasst_Vertices_Koord.csv")
...
for (graben in 1:length(grabenList)) {
  ...
  h_list <- c()
  for (grabenZellen in 1:length(grabenList[[graben]][,2])) {
    h_0 <- dem[grabenList[[graben]][grabenZellen,2], grabenList[[graben]][grabenZellen,1]]
    h_list <- append(h_list, h_0)
  }
  h <- mean(h_list) - 0.3
...
writeRaster(dem,
            "S:/DGM_Flow_Modellierung/07_data_preprocessing/QGIS_Projekte/Argenschwang_Zwischengräben.tif",
